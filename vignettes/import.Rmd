...
[Variant Call Format (VCF)](https://samtools.github.io/hts-specs/) files have become a standard for genomic data. This is the storage format used for the [1000 genomes](https://www.internationalgenome.org/) for instance. VCF files can be imported in `hierfstat` using the function `read.VCF` , build from the `gaston` package function `read.vcf`:
...
by default, `read.VCF` keeps only the bi-allelic SNPs (whereas `gaston::read.vcf` all variants), but you can choose to keep all variant by setting the argument `BiAllelic` to FALSE.  

`as.matrix(x1)` will then provide allelic dosages, suitable for analysis with several of the `hierfstat` functions. 
