...
Variant Call Format (VCF) files have become a standard for genomic data. This is the storage format used for the [1000 genomes](https://www.internationalgenome.org/) for instance. Importing VCF files can be done via several routes.

* The easiest is via the R package [gaston](https://cran.r-project.org/web/packages/gaston/index.html):
...
In this example, the VCF file contains only bi-allelic SNPs, but if it is not the case, some extra steps are needed to make sure only bi-allelic SNPs are kept, by pruning out the multi-allelic sites. `as.matrix(x1)` will then provide allelic dosages, suitable for analysis with several of the `hierfstat` functions. 


* Another route is via the `SNPRelate` package, and its function `snpgdsVCF2GDS`. As the function writes to a fil, the two next code chunks will not be evaluated. 
