...
`hierfstat`can now export to files suitable for [Bayescan](http://cmpg.unibe.ch/software/BayeScan/), [plink](https://www.cog-genomics.org/plink/2.0/) and [structure](https://web.stanford.edu/group/pritchardlab/structure.html). Functions to import from these program can be found in `adegenet`. The functions to export to these programs are `write.bayescan`, `write.ped` and `write.struct` respectively.
